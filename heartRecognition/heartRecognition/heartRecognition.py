...
def main():
    ...
    path = 'C:\\Repos\\mech423final\\heartRecognition\\heartRecognition\\trainingDataHeart'
    i = 0
    y_data = numpy.array([])
    ...
    model = Sequential()
    model.add(LSTM(16, input_shape=(None,1), return_sequences=True)) #the 256 is semi-arbitrary?
    model.add(LSTM(16))
    model.add(Dense(1)) 
    ...
    model.save('lie_detector_model_heart.h5');
    print(loss)
    # 5. make predictions
